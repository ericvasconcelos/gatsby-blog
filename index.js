  ...
  return (
    ...
    >
      <div className="mb-6 mt-4">
        <button className="absolute right-4 top-2" onClick={() => setOpenModal(false)}>
          <img className="w-6" src={CloseIcon} alt="close icon" />
        </button>
        <ProductSearch searchQuery={searchQuery} setSearchQuery={setSearchQuery} openModal={openModal} />
      </div>
      {results.length > 0 && (
        <div>
      ...
      {hasNoResults && <p className="italic text-sm font-light mt-2">Your search didn't return any results</p>}
    </div>
  );
};
